...
x_train <-bind_cols(Subjects = subject_train.txt$V1, Activity = y_train$Activity, X_train.txt)
x_test<-bind_cols(Subjects = subject_test.txt$V1, Activity = y_test$Activity, X_test.txt)

# 1. Merges the training and the test sets to create one data set.

allData <- bind_rows(x_train,x_test)
